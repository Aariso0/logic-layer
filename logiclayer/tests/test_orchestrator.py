...
@patch('logiclayer.verifier.orchestrator.check_local_db', new_callable=AsyncMock)
@patch('logiclayer.verifier.orchestrator.search_trusted_sources', new_callable=AsyncMock)
@patch('logiclayer.verifier.orchestrator.report_verdict', new_callable=AsyncMock)
async def test_local_db_hit_prevents_web_search(
    ...
    mock_db.assert_called_once_with("The moon orbits the earth.")
...
async def test_fallback_to_trusted_sources(
    ...
    mock_db.assert_called_once_with("Deep learning relies on backpropagation.")
    mock_search.assert_called_once_with("Deep learning relies on backpropagation.")
